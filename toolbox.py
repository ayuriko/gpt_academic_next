...
# Gradio shim — provide minimal stubs so existing code works without gradio installed
try:
    import gradio
except ImportError:
    class _GradioShim:
...
def write_history_to_file(
    ...
    if file_fullname is None:
        if file_basename is not None:
            ...
            file_fullname = pj(get_log_folder(), f"GPT Academic Next-{gen_time_str()}.md")
    os.makedirs(os.path.dirname(file_fullname), exist_ok=True)
    with open(file_fullname, "w", encoding="utf8") as f:
        f.write("# GPT Academic Next Report\n")
