    ...
    def main_process_ui_control(self, txt, create_or_resume) -> str:
        ...
        while True:
            ...
            if self.parent_conn.poll():
                ...
                if "[GPT Academic Next] 等待中" in self.chatbot[-1][-1]:
                ...
                self.feed_heartbeat_watchdog()
                if '[GPT Academic Next] 等待中' not in self.chatbot[-1][-1]:
                    # begin_waiting_time = time.time()
                    self.chatbot.append(["[GPT Academic Next] 等待AutoGen执行结果 ...", "[GPT Academic Next] 等待中"])
                self.chatbot[-1] = [self.chatbot[-1][0], self.chatbot[-1][1].replace("[GPT Academic Next] 等待中", "[GPT Academic Next] 等待中.")]
                yield from update_ui(chatbot=self.chatbot, history=self.history)
