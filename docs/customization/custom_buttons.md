...
在日常使用中，您可能会发现某些特定的提示词组合使用频率很高——比如将文本翻译成特定的学术风格、按照固定格式生成代码注释、或者用专业术语润色技术文档。GPT Academic Next 提供了自定义按钮功能，让您可以把这些常用的提示词模板固化为界面上的快捷按钮，一键触发专属的 AI 工作流。
...
在 GPT Academic Next 主界面的左下角，找到 **界面外观** 下拉菜单并点击展开。在弹出的浮动面板中选择 **自定义菜单** 选项，这会打开按钮配置界面。
...
修改完成后保存文件，重新启动应用即可看到新按钮。由于 GPT Academic Next 支持热重载，如果在 `config.py` 中启用了 `PLUGIN_HOT_RELOAD = True`，部分修改无需重启即可生效。
