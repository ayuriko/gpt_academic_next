...
本文档详细介绍 GPT Academic Next 的配置系统和各项配置参数。通过合理配置，您可以充分发挥项目的潜力，打造符合自己需求的工作环境。
...
GPT Academic Next 采用三层配置机制，读取优先级从高到低依次为：
...
??? info "支持的模型前缀"
    除了标准模型名称，GPT Academic Next 还支持以下前缀格式接入更多模型：
