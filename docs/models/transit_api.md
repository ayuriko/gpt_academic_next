...
在某些场景下，您可能无法直接访问 OpenAI 等模型提供商的官方 API，或者希望通过统一的接口管理多种模型。这时，使用第三方中转服务是一个实用的解决方案。本指南将详细介绍如何在 GPT Academic Next 中配置和使用中转渠道。
...
GPT Academic Next 提供了三种灵活的中转接入方式，您可以根据实际需求选择：
...
配置完成后，启动 GPT Academic Next 并进行测试：
