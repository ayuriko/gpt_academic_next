...
本文将引导您完成 Azure OpenAI 服务的申请和配置。整个过程包括在 Azure 平台创建资源、部署模型，以及在 GPT Academic Next 中配置连接参数。
...
完成配置后，启动 GPT Academic Next 验证连接是否正常：
...
???+ question "提示 Rate limit exceeded"
    ...
    1. 在 Azure 门户中申请提升配额
    2. 在 GPT Academic Next 中降低并发请求数
    3. 升级 Azure 订阅等级
