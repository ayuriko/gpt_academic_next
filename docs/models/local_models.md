...
在云端 API 之外，GPT Academic Next 还支持接入完全运行在本地的大语言模型。本地部署意味着您的所有数据都不会离开您的计算机——这对于处理敏感的学术论文、商业代码或私密对话来说尤为重要。此外，本地模型不受 API 配额限制，一旦部署完成便可无限制使用。
...
完成配置后，启动 GPT Academic Next 验证本地模型是否正常工作：
...
<!-- 描述: GPT Academic Next 界面截图，展示模型下拉菜单选择本地模型（如 ollama-llama3.2），以及一次成功的对话 -->
<!-- 标注: 用红框标注模型下拉菜单位置 -->
<!-- 尺寸建议: 1000px -->
