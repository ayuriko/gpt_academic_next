...
学术论文中的数学公式一直是 PDF 翻译的难点——传统的文本提取方法往往将公式渲染成乱码或直接跳过。Meta AI 开发的 NOUGAT（Neural Optical Understanding for Academic Documents）模型专门针对学术文档设计，能够将 PDF 中的内容（包括复杂的数学公式）高质量地转换为结构化的 Markdown 格式。GPT Academic Next 集成了 NOUGAT 解析能力，为数学和物理等公式密集型论文提供了更精准的翻译方案。
...
使用此功能前，需要在 GPT Academic Next 运行环境中安装 NOUGAT 及其相关依赖：
...
将需要翻译的 PDF 论文上传到 GPT Academic Next 界面。您可以上传单个文件，也可以将多个 PDF 文件放入文件夹后打包上传进行批量处理。上传完成后，文件路径会自动填入输入框。
...
???+ question "NOUGAT 模型下载失败"
    ...
    2. 在 GPT Academic Next 配置文件中设置 `proxies` 代理参数
