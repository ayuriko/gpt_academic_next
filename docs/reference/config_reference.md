...
本文档是 GPT Academic Next 所有配置项的完整参考手册。在实际使用中，您通常只需关注与当前需求相关的配置项。如果您是首次配置，建议先阅读 [快速上手](../get_started/quickstart.md) 和 [配置详解](../get_started/configuration.md)，本手册更适合作为查阅工具使用。

---

## 配置优先级

GPT Academic Next 支持三种配置方式，系统按以下优先级读取配置（高优先级会覆盖低优先级的同名配置）：
...
GPT Academic Next 支持通过前缀接入各种兼容服务：
